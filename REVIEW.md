...
# 🔁 دور سوم — باگ انیمیشن آلبوم‌ها (گزارش کاربر)

### ۱۳. آلبوم‌ها «یهو میان و غیب می‌شوند» + welcome پخش نمی‌شد
دو مشکل هم‌زمان:
- **ناپدید شدن کارت‌ها:** HTML استاتیک کارت‌های `.rv` داشت که fx.js مشاهده‌شان می‌کرد و visible
  می‌شدند؛ اما بعد `albums.js` با داده‌ی API کل گرید را replace می‌کرد — کارت‌های جدید `.rv`
  (یعنی opacity:0) بودند و **دیگر هرگز observe نمی‌شدند** → غیب. همان مشکل در «محصولات
  ویژه»ی صفحه‌ی اصلی هم بود.
  **فیکس:** fx.js حالا با MutationObserver المنتهای `.rv` که بعداً به DOM اضافه می‌شوند را هم
  observe می‌کند (انیمیشن reveal حفظ شده).
- **welcome آلبوم‌ها:** albums.html و index.html **هر دو از یک کلید localStorage** استفاده
  می‌کردند (`or_welcomed_albums_liquid_void`) → با یک بازدید از خانه، انیمیشن welcome-pilot
  آلبوم‌ها برای همیشه غیرفعال می‌شد.
  **فیکس:** کلید جدا برای صفحه‌ی آلبوم‌ها (`or_welcomed_pilot_liquid_void`).

**راستی‌آزمایی:** شبیه‌سازی واقعی DOM (jsdom) علیه سرور زنده — هر دو سناریو (با/بدون پرچم
welcome) + صفحه‌ی اصلی: ۴ کارت آلبوم همه visible، welcome مستقل پخش می‌شود، محصولات
ویژه و دیسکوگرافی سالم.
